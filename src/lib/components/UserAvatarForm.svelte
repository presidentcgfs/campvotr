<script lang="ts">
  ...
  async function upload() {
    error=''; success='';
    if (!file) { error = 'Please choose a file.'; return; }
  ...
  async function removeAvatar() {
    ...
    finally { removing = false; confirmOpen = false; }
  }
  ...
  async function mapError(res: Response) {
    ...
    if (res.status === 409 || res.status === 422) return new Error(body?.error || 'Invalid image. Please upload a PNG/JPG/WEBP up to 2MB.');
  ...
  {:else}
    <div class="row">
      ...
      <div class="controls">
        ...
        <input id="avatar-file" type="file" accept="image/png,image/jpeg,image/webp" on:change={(e) => file = (e.target as HTMLInputElement).files?.[0] ?? null} disabled={saving || removing} />
        <div class="actions">
          <button class="btn" on:click={upload} disabled={saving || removing || !file}>Upload</button>
          <button class="btn" on:click={askRemove} disabled={saving || removing || !avatar_url}>Remove</button>
        </div>
...
<style>
  .row { display: grid; gap: 1rem; grid-template-columns: 110px 1fr; align-items: center; }
  .avatar { width: 96px; height: 96px; border-radius: 9999px; object-fit: cover; border: 1px solid #e5e7eb; }
  .avatar.placeholder { display:flex; align-items:center; justify-content:center; background:#f3f4f6; color:#6b7280; font-weight:600; font-size: 24px; }
