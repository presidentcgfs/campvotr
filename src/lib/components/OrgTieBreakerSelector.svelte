<script lang="ts">
	...
	async function load() {
		...
		try {
			const [mres, tres] = await Promise.all([
				...
				fetch(`/api/organizations/${orgId}/tie-breaker`)
			]);
	...
	async function save() {
		...
		success = '';
		try {
			const res = await fetch(`/api/organizations/${orgId}/tie-breaker`, {
				method: 'POST',
...
<div class="tiebreaker">
	...
	{:else}
		...
		{/if}
		<label
			>Designated tie-breaker
			<select
				bind:value={tieBreakerUserId}
				disabled={!canEdit || saving}
				aria-label="Designated tie-breaker"
			>
